...
def hemoglobin_pdf_from_mean_sd(mean, sd):
    ...
    def pdf(x):
        return (
            GAMMA_DISTRIBUTION_WEIGHT
            * hemoglobin_distribution_gamma_part.pdf(x)
            + MIRROR_GUMBEL_DISTRIBUTION_WEIGHT
...
def hemoglobin_sampler_from_mean_sd(mean, sd):
    ...
    def sampler(distribution_propensity, propensity):
        ...
        propensity = np.clip(propensity, 0.001, 0.999) # HACK: risk_distributions is way too conservative!
        result[distribution_propensity < GAMMA_DISTRIBUTION_WEIGHT] = hemoglobin_distribution_gamma_part.ppf(propensity)[distribution_propensity < GAMMA_DISTRIBUTION_WEIGHT]
        result[distribution_propensity >= GAMMA_DISTRIBUTION_WEIGHT] = hemoglobin_distribution_mgumbel_part.ppf(propensity)[distribution_propensity >= GAMMA_DISTRIBUTION_WEIGHT]
        return result
...
def _hemoglobin_distribution_parts_from_mean_sd(mean, sd):
    ...
    kwargs = {}
    if not isinstance(mean, pd.Series) and not isinstance(sd, pd.Series) and not isinstance(mean, np.ndarray) and not isinstance(sd, np.ndarray):
        kwargs["index"] = [0]
